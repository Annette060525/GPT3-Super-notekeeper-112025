...
FLOWER_THEMES = [
    # id, label_en, label_zh, primary, secondary, bg_gradient_light, bg_gradient_dark
    {
        "id": "sakura",
...
def init_session_state():
    defaults = {
        ...
        "user_prompt": "",
        "runs_log": [],   # list of dicts with run stats
        "notes": "",
    }
...
def load_agents_config(provider: str, model: str, max_tokens: int, temperature: float) -> Dict[str, Any]:
    """
    Load agents.yaml and inject dynamic model parameters
    without hardcoding the model into the YAML.
    """
...
def page_agents_console():
    ...
    st.markdown(
        ...
        <div class="flower-card">
            ...
            <p style="font-size:0.85rem;opacity:0.8;">
                Configure your providers, models, prompts and run multi-agent workflows powered by <code>agents.yaml</code>.
            </p>
        </div>
        """,
        unsafe_allow_html=True,
    ...
    with col_side:
        ...
        # Sync system prompt when template is changed
        tmpl = get_prompt_template_by_id(st.session_state.prompt_template_id)
        if st.session_state.system_prompt == "" or st.session_state.system_prompt == PROMPT_TEMPLATES[0]["system_prompt"]:
    ...
    if run_button:
        ...
        with st.spinner("Executing agents workflow..."):
            ...
            # TODO: integrate your real agent runner here.
            # This block is a placeholder to show how you'd plug values in.
            start = time.time()
            simulated_latency = 0.8
            time.sleep(simulated_latency)
            approx_tokens = min(max_tokens, 512)

            latency = time.time() - start
            record_run(provider, model, tokens=approx_tokens, latency=latency)

        st.success("Agents executed (placeholder). Plug in your real agent executor here.")
        st.code(
            {
                "provider": provider,
                "model": model,
                ...
                "agents_yaml_loaded": bool(agents_cfg),
            },
            language="json",
        )
...
def page_attachment_chat():
    ...
    st.markdown(
        ...
        <div class="flower-card">
            ...
            <p style="font-size:0.85rem;opacity:0.8;">
                Upload documents or files and chat with them using the same model settings as the Agents Console.
                Connect this UI to your embedding or RAG pipeline for full functionality.
            </p>
        </div>
        """,
        unsafe_allow_html=True,
    ...
    with col_chat:
        ...
        if ask_button:
            with st.spinner("Thinking about your documents..."):
                time.sleep(0.5)
            st.info(
                "This is a placeholder. Connect your RAG / document QA logic here, "
                "using the provider/model settings from the sidebar."
            )
...
def build_dashboard_dataframe() -> pd.DataFrame:
    if not st.session_state.runs_log:
        return pd.DataFrame(
            columns=["timestamp", "provider", "model", "tokens", "latency"]
        )
...
def page_dashboard():
    ...
    st.markdown(
        ...
        <div class="flower-card">
            ...
            <p style="font-size:0.85rem;opacity:0.8;">
                Track how your agents are being used over time: runs, tokens, latency and model mix.
            </p>
        </div>
        """,
        unsafe_allow_html=True,
    ...
    if df.empty:
        st.info("Run at least one agent execution to populate the dashboard.")
        return
...
def sidebar():
    ...
    with st.sidebar:
        ...
        # Model settings (read-only here, detailed controls are in Agents page)
